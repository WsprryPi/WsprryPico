#!/usr/bin/env python3
"""Prove that Package 9's raw auditor rejects closure-critical mutations."""
...
from audit_phase11_5_package9 import audit
...
def mutations():
    ...
    return [
        ...
        ("finite-rf-budget", packet("rf_jobs", 12)),
        ...
        ("runtime-source-impact", lambda c: change_json(c["root"] / "packet.json",
            lambda value: value["source_impact"].__setitem__("later_runtime_source_changes", 1))),
        ("frozen-helper-hash", lambda c: change_json(c["root"] / "packet.json",
            lambda value: value["stage_sha256"].__setitem__(
                "scripts/phase11_5_package9.py", "0" * 64))),
        ...
        ("normalizer-count", lambda c: change_jsonl(c["root"] / "campaign.jsonl",
...
def run(root, packet_sha, decoder, fixture_root, reservation):
    ...
    for name, change in mutations():
        with tempfile.TemporaryDirectory(prefix="phase11-5-package9-adversarial-") as temp:
            base = Path(temp)
    ...
    return {"schema": "phase11.5-package9-adversarial-v1", "status": "PASS",
