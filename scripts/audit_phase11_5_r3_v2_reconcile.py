...
def audit(root):
    require(digest(root/'packet.json')==PACKET,'Frozen R0a packet')
    ...
    require(rows[0]['kind']=='start' and rows[0]['value']==dict(packet_sha256=PACKET,needed=True) and
    ...
    require(r['status']=='FAILED' and r['after-b-error']=="'launch_epoch'" and 'error' not in r and
        r['rf_jobs']==r['rf_duration_ns_charged']==0 and r['claim_attempts']==r['release_attempts']==1 and
        r['after-a']==values['after-a']['wtp']['STATUS'],'Original comparator-field failure retained')
    return dict(status='IDLE_RECONCILIATION_INDEPENDENTLY_VERIFIED',packet_sha256=PACKET,
        source_revision=p['source_revision'],image_sha256=p['image_sha256'],boot_id=p['boot_id'],
        final_a=values['after-a']['wtp']['STATUS'],final_b=values['after-b']['wtp']['STATUS'],
        b_unchanged=True,configuration_unchanged=True,rf_counters_unchanged=True,rf_jobs=0,
        claim_attempts=1,release_attempts=1,
        original_runner_status='FAILED',preserved_finding='R0a assumed RF telemetry on the inhibited B image')
