#!/usr/bin/env python3
"""Recover the known completed browser/USB-reducer Phase 11.6 run offline."""
...
from phase11_6.recovery import recover_browser
...
def main() -> int:
    ...
    if job["id"] != args.job_id or not job["submission_path"].startswith("browser"):
        raise ValueError("Exact browser recovery job")
    result = recover_browser(
    ...
    print(json.dumps({"recovered": True, "job_id": result["job_id"],
                      "capture_sha256": result["capture"]["capture_sha256"]}, indent=2))
    return 0
