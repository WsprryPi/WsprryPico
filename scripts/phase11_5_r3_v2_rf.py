...
def completion_policy(packet):
    return packet.get('closure_policy') in (COMPLETION_POLICY,TERMINAL_POLICY,READ_POLICY,HTTP_POLICY,SHARING_POLICY)
...
def validate(packet):
    ...
    http_repair=packet.get('closure_policy') in (HTTP_POLICY,SHARING_POLICY)
    if http_repair:
        ...
        require(packet.get('capacity_schedule_policy')==SERIAL_CAPACITY and
                ...
                packet['boot_id']==(SHARING_BOOT if sharing_repair else HTTP_BOOT) and isinstance(ids,list) and len(ids) in (0,3,5) and
    ...
    closure=packet.get('closure_policy') in (CLOSURE_POLICY,REPAIR_POLICY,COMPLETION_POLICY,TERMINAL_POLICY,READ_POLICY,HTTP_POLICY,SHARING_POLICY)
    ...
    if closure:
        require('diagnostic_policy' not in packet and packet['source_revision']==(SHARING_SOURCE if sharing_repair else HTTP_SOURCE if http_repair else READ_SOURCE if read_repair else TERMINAL_SOURCE if terminal_repair else COMPLETION_SOURCE if completion else REPAIR_SOURCE if repair else DIAGNOSTIC_SOURCE) and
...
def run(root,packet):
    diagnostic=packet.get('diagnostic_policy')==DIAGNOSTIC_POLICY
    capture_failures=diagnostic or packet.get('closure_policy') in (CLOSURE_POLICY,REPAIR_POLICY,COMPLETION_POLICY,TERMINAL_POLICY,READ_POLICY,HTTP_POLICY,SHARING_POLICY,PRESSURE_POLICY)
    ...
    try:
        ...
        if reservation is not None:
            ...
            if packet.get('closure_policy') in (HTTP_POLICY,SHARING_POLICY):
