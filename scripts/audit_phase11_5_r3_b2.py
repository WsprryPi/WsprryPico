"""Independent B2 transport acceptance; never full R3 closure."""
import json
...
from audit_phase11_5_r3_transport import audit as audit_tls

PACKET='b41cca02d4f6f34e773eed5e5d04b292104b184a2e30c318e5713d1c529a33a0'


def audit(root):
    require(digest(root/'packet.json')==PACKET, 'Different B2 packet')
    ...
    require(result['pressure']['status']=='PASS' and len(result['pressure']['cases'])==14 and
            result['completed_jobs']==2 and result['family_closed'] is False, 'Incomplete B2 subset')
    ...
    for name in order:
        ...
        if b:
            ...
            if name!='final-terminal-before':
                require(value['wtp']['STATUS']['terminal_records']==baseline['wtp']['STATUS']['terminal_records']==rf['prior_terminal_records'] and
    ...
    return dict(status='PASS_B2_TRANSPORT_SUBSET',packet_sha256=PACKET,accepted_b2_pressure_cases=14,
                ...
                final_status=values['final-terminal-before']['wtp']['STATUS'],raw_b2=result,
                limitation='TLS alert, pending expiry and HTTP partial/reader paths only; remaining R3 register stays open')
