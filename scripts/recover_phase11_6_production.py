#!/usr/bin/env python3
"""Recover a completed Phase 11.6 production run without hardware access."""
...
from phase11_6.recovery import recover
...
def main() -> int:
    ...
    if job["id"] != args.job_id or job["submission_path"] != "production":
        raise ValueError("Exact production recovery job")
    result = recover(
        args.plan.resolve().parent, plan, job, attempt, args.attempt_packet,
        args.attempt_root, args.reconciliation,
        args.capture_helper.resolve(strict=True), args.qualification_src,
        args.production_binary.resolve(strict=True), args.output,
    )
    print(json.dumps({
        "recovered": True,
