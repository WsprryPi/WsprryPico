...
firmware correction below passes host regression and image checks; flashing
that new image and its physical comparison await explicit authorization.
...
requires captured AP beacons and actual AP/client data, followed by Pico
association capture. The completed capture contains 7,742 valid decoded frames;
the serial trace contains contiguous events 1–152. Neither capture completeness
nor successful radio delivery is inferred from a driver's submission result.
...
Ethernet is retained as the independent management path.
...
name `wsprrypico-0a60df.local`. The observed standard inhibited image reports
`dbf1d86f0885-dirty`, SDK 2.3.1, and a 150 MHz system clock.

- Native Linux discovery, authenticated WTP STATUS and HTTPS status passed at
  `10.77.14.10` before the lease change.
- Captured DHCP REQUEST/ACK transaction `653b90fa` assigned `10.77.14.20`.
  Boot `02b19ddb0fce76afe3f525566ae8728d` remained unchanged.
- The installed WsprryPi production binary authenticated the same Pico at both
  ...
  successful responses, zero LOAD/ARM operations, and the same certificate
  fingerprint. These were fresh production connections, not a demonstrated
  continuously running production-client recovery across the transition.

## Firmware correction and review

`PicoNetwork::poll()` now preserves Wi-Fi association while the SDK reports
...
The physical trace made this defect actionable: the Pico generated a SYN-ACK
...
remaining actionable source/test finding in this slice. Physical verification
of the candidate remains outstanding.
...
existing MAC-suffix credentials are preserved. No candidate flash is claimed.

## Restored state and remaining acceptance

Pico A is back on Bohica-IoT at `.47`, boot
`b22b2bf4a58c4217c34cee90a30bc433`; comparator B remains at `.53`, boot
...
| --- | --- | --- |
| B2 — Linux discovery/reconnection | Open | Preserve the working setup; verify repeatable physical recovery with the correction where applicable. |
| D1 — DHCP address change | Open; correction host-tested | Flash the reviewed inhibited image after authorization; verify new-address reference recovery and native Mac/production recovery without relaxing deadlines. |
