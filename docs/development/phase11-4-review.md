...
retains a missing-probe capture and unexplained historical recovery failure.
Normal Chrome navigation and in-page refresh were observed working with
Anti-tracker enabled; the automated-navigation block is separate.
...
bounded scope. Chrome reported ERR_BLOCKED_BY_CLIENT; browser recovery and
intermittent discovery remained unresolved at that attempt; see the newer B2/D2
record for ordinary Chrome recovery.
...
address error, so D2 remains partial. The subsequent D3 record above covers
the selective physical fault.
