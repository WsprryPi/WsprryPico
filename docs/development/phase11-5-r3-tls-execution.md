...
`phase11.5-r3-tls-a1-v1`. Its [preparation review](phase11-5-r3-tls-review.md)
and [machine-readable record](phase11-5-r3-tls-prepared.json) distinguish
implemented tooling from unrun physical acceptance.
...
- Expected initial inhibited A boot `587c672d4267e467649bb43765542284`,
...
- New private root `/home/pi/phase11-5-r3-tls-a1-20260912`, mode 0700.
  Existing roots or partially staged attempts cannot be overwritten or replayed.
- Local archive `build/phase11-5-r3-tls-a1/staging.tar`: 4,689,920 bytes, 60 files.
  SHA-256 `7fbc07f5d787f5f71ee4e491dfcda296b26f19b724ec8b5f445fe8bf7984ca6e`.
- Packet SHA-256 `cab1abf2afffa27d79992b1d84bf393a2f1059ad3a4615829407485d695ca7a4`.
...
`/home/pi/phase11-5-r3-tls-a1-20260912.tar` and
`/home/pi/phase11-5-r3-tls-a1-20260912-stage.py`, without overwriting existing files.
...
`/home/pi/phase11-5-r2-upload-2e43110`: original UF2/configuration, the native
...
```sh
sudo python3 /home/pi/phase11-5-r3-tls-a1-20260912-stage.py \
  --archive /home/pi/phase11-5-r3-tls-a1-20260912.tar \
  --root /home/pi/phase11-5-r3-tls-a1-20260912 \
  --archive-sha256 7fbc07f5d787f5f71ee4e491dfcda296b26f19b724ec8b5f445fe8bf7984ca6e \
  --packet-sha256 cab1abf2afffa27d79992b1d84bf393a2f1059ad3a4615829407485d695ca7a4 --run
```

Then execute once on wspr5, preserving stdout/stderr in the new evidence root:

```sh
sudo python3 /home/pi/phase11-5-r3-tls-a1-20260912/scripts/phase11_5_r3_tls.py \
  --root /home/pi/phase11-5-r3-tls-a1-20260912 \
  --packet-sha256 cab1abf2afffa27d79992b1d84bf393a2f1059ad3a4615829407485d695ca7a4 --run
```

A packet field saying `fixture_authorized` or `current_wiring_confirmed` is a
condition of this proposed grant, not independent evidence of user approval.
Do not execute these commands on the strength of those fields.
...
PIO/DMA engine. No rebuild or firmware implementation change accompanies A1.
...
The new grant must confirm that the historical conducted path is still present:
...
| --- | --- | --- | --- |
| A1-1 | Sole USB observer/actor | Tone, 135,500 Hz, 100 seconds | `80a8779729a90e0b7a3230b6198a7a9d` |
| A1-2 | Same USB actor | Tone, 135,500 Hz, 100 seconds | `f3eba63a34a27811116c09d7b14ae1c3` |
...
| --- | --- |
| A1-1 positive | Fresh TLS 1.3, verified target certificate, HTTP/1.1 GET `/api/v1/status`, correct owner/job/boot; two active connections, zero pending |
...
| A1-2 positive | Fresh authenticated HTTPS while the same production WTP connection continues |
...
pending sockets are distinct: A1 does **not** claim pending expiry, failed-alert
...
case requires fresh INFO and STATUS evidence of owned Running RF. The offline
...
Carry **CONFIG 34 forward to a maximum of 36**, with probes unchanged at six.
...
services. Counts must finish at CONFIG 36 and probes six.
...
staging, the listed fixture/device operations, CONFIG 34→36, and the unchanged
conducted wiring. Local implementation, tests and publication were already
