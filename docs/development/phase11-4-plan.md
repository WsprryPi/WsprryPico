...
Status: **OPEN**. Physical results belong to the single matrix below; software
checks cannot close its gates. This plan coordinates WsprryPico and WsprryPi.
...
5. **LAN/conflict:** real DHCP reassignment must be limited to this Pico lease on
   an identified router; never edit hosts, reserve an IP or disrupt wspr5's LAN.
...
| A2 device UTC gating | Candidate + USB/network reads | Record SNTP and GET_CLOCK before TLS, reject unsynchronized or excessive-uncertainty dependent jobs; no time falsification | `postflash.*`, `final-usb.*` positive SNTP/GET_CLOCK; BLOCKED negative dependent-job authorization | Positive usable clock observed; no time falsification or dependent-job negative test |
...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound `getent ahostsv4` to 15 s; production getaddrinfo belongs to B3; optional resolver utilities separately, absent tools not PASS | `linux-getent.*` baseline PASS; short-name success after Wi-Fi cycle followed by `final-linux-nss.*` exit 2; FAIL current short-name NSS | Original short-name exit 2 retained; recovery was temporary; optional resolver utilities absent |
| B3 production integration | Approved isolated Linux build/config/invocation | Hash real executable, configure exact endpoint/CA/controller/device; observe fresh resolved/authenticated/WTP identities and startup inspection | Isolated build PASS; B3 BLOCKED invocation authorization and installed-service singleton conflict | Archive build failed for missing Git metadata; full-bundle clone build passed; no installed-service change |
...
| C2 concurrent status/authority | C1+B3+approved finite jobs | Persistent WTP plus browser reads in Loaded/Armed/Running; foreign mutation rejected, owner accurate; failed read becomes unknown | pending; BLOCKED prerequisites | Preserve host idle-management policy / pending |
| C3 revisions/drafts/secrets | Approved exact idle config/schedule delta | Save/load with ETag, stale 412 and missing 428, preserve drafts and redaction through both paths; compare saved config | pending; BLOCKED config authorization | None / pending |
| C4 deferred network change | Approved idle Wi-Fi off/on | Observe complete response ACK before application, aborted response cancels, recheck idle; restore with Console | BLOCKED browser network-mutation authorization; Console-only cycle does not test deferred HTTP application | None / pending |
| D1 real DHCP reassignment | Router/lease access and packet approval | Record old/new lease, same name/CA/server fingerprint/device; peer resolution and device advertisements use new IP, no duplicate submission | pending; BLOCKED router details/authorization | None / pending |
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | BLOCKED privileged capture; one authorized Console cycle passed (`wifi-off.*`, `wifi-on.*`, `wifi-postcheck.*`) | `sudo -n tcpdump` required a password; no capture; one goodbye send attempt is not reception |
| D3 actual link loss | Approved Pico-only link fault | No successful goodbye claim after loss; record stale-peer TTL, recovery/address/state, existing job/owner unchanged | pending; BLOCKED fault mechanism/authorization | None / pending |
| E1 distinct boards/trust | Two separately authorized boards/CAs | Independent default names/device IDs and per-board trust rejection | pending; BLOCKED second board unavailable | Host simulation cannot close / pending |
| E2 alias conflict during job | Approved second board or exact alternative responder | Certified alias conflict; record actual winner, loser conflict/empty advertised name/no suffix; finite inhibited job continues | pending; BLOCKED conflict mechanism/authorization | Alternative is narrower than E1 / pending |
| E3 conflict recovery | E2 + approved idle retry | Remove conflicting claimant, WIFI OFF/ON, same certified name probes active and owner remains coherent | pending; BLOCKED E2 | None / pending |
| F1 server renewal | New same-CA bundle + approved exact reflash | Same device/DNS identity/CA, new fingerprint/validity, existing valid clients and DHCP workflow continue | BLOCKED same-hostname renewal candidate/flash authorization | Alias-change renewal preserved CA/client trust but changed DNS name; not this unchanged-name case |
| F2 client replacement | Separate new controller/browser/import approval | Rotate after authoritative idle cleanup; replacement works, old valid identity still trusted (issuance is not revocation) | pending; BLOCKED credential deployment/import | CA replacement is compromise recovery / pending |
| F3 identity failures | Approved bounded negative TLS requests | Wrong hostname/CA, missing/invalid/expired client, IP without SAN fail at intended client/device layer; no WTP mutation | pending; BLOCKED physical network access | Expired server build rejects before flash; never disable validator / pending |
| F4 wrong-board deployment | Separately approved mismatched image | Manifest/certificate internally valid but board ID differs; INFO false, listener/name fail closed, output remains inhibited | pending; BLOCKED exact negative image authorization | Build rejection alone is not runtime gate evidence / pending |
| F5 IP with DNS identity | Approved production TLS/HTTP reads | Known IP + expected DNS authenticates DNS SAN and hostname Host/Origin; independent WTP device check | BLOCKED production invocation; supplementary OpenSSL GET passed (`final-ip-dns-identity.*`), USB identity independently verified | Earlier IP/short-name timeout retained; OpenSSL is not the Pi application and not a substitute for B1/B2 |
| F6 literal IP SAN success | Approved exact IP-SAN image/client URL | Exact current IP SAN authenticates browser URL and numeric HTTP authority | pending; BLOCKED IP-SAN candidate/flash | Base candidate intentionally has no IP SAN / pending |
| G1 finite completion | Approved bounded host and direct-browser jobs | Complete CAPS-valid LOAD before ARM; matching job terminal STATUS and explicit inactive output | pending; BLOCKED job authorization | No physical timing/RF claim / pending |
| G2 cancellation/foreign owner | Approved bounded jobs and owner mutations | Loaded/Armed/Running ABORT where supported; foreign ABORT/RELEASE rejected; explicit owner cleanup | pending; BLOCKED G1 | None / pending |
| G3 loss after acknowledged ARM | Approved endpoint-only connection cut | Device completes autonomously; reconnect same session with HELLO+STATUS, retain ownership/terminal proof | pending; BLOCKED fault authorization | Disconnect is not inactive proof / pending |
| G4 lost LOAD/ARM/ABORT replies | Audited bounded response-loss mechanism | Prove intended request reached device and reply was withheld; original request ID/bytes replay or STATUS reconciliation, no duplicate execution | pending; BLOCKED injection design/authorization | Opaque TCP cut alone insufficient / pending |
| G5 failed DNS/TLS reconnect | Approved isolated controller fault | Retain output unknown and failed history; no additional LOAD/ARM; restore and reconcile using original identity | pending; BLOCKED controller fault authorization | No LAN-wide DNS/trust change / pending |
| G6 process/boot/device changes | Approved test-process restart, board restart/identity mechanism | New process session cannot adopt old work; boot/device change blocks resubmission and retains failures | pending; BLOCKED restart/fault authorization | Wrong board needs independent identity case / pending |
| G7 USB versus Console recovery | Explicit owned USB recovery and Console override | USB principal cannot steal TLS job; separately approved Console ABORT suspends scheduling and confirms disable/release | pending; BLOCKED mutation authorization | Existing read-only USB approval insufficient / pending |
| H1 persistence | Approved saved delta and restart/power cycle | Config/schedule/watermark retained, no repeat, fresh boot; compare before/after and terminal evidence before reset | pending; BLOCKED persistence/restart authorization | Watermark is reservation, not completion / pending |
| H2 final cleanup | Approved cleanup set + read-only verification | Exact image/boot/device, inactive STATUS/unowned, scheduling disabled/history preserved, Wi-Fi/name and client/trust/service states recorded | `final-usb.*`, `final-linux.*`, browser observation; PASS cleanup of executed scope only | Remaining unexecuted matrix stays open; original service/hash/PID unchanged; no RF image restore |
