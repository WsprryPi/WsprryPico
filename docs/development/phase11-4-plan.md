...
| A2 device UTC gating | Candidate + USB/network reads | Record SNTP and GET_CLOCK before TLS, reject unsynchronized or excessive-uncertainty dependent jobs; no time falsification | PARTIAL: positive SNTP and 1 ns CLOCK_UNCERTAIN rejection passed in `jobs-attempt-1`; boot-unsynchronized USB observed | No time falsification; unsynchronized dependent-job subcase remains unexecuted |
...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound `getent ahostsv4` to 15 s; production getaddrinfo belongs to B3; optional resolver utilities separately, absent tools not PASS | Intermittent PASS/FAIL: production/NSS short-name success observed this run; final cycle NSS timed out | Retain baseline pass, original exit 2, later success and current failure; no reliability claim |
...
| C2 concurrent status/authority | C1+B3+approved finite jobs | Persistent WTP plus browser reads in Loaded/Armed/Running; foreign mutation rejected, owner accurate; failed read becomes unknown | PARTIAL: direct HTTPS observed Loaded owner; foreign ABORT/RELEASE rejected; production armed/running history retained | Actual concurrent Chrome Loaded/Armed/Running sequence unexecuted |
| C3 revisions/drafts/secrets | Approved exact idle config/schedule delta | Save/load with ETag, stale 412 and missing 428, preserve drafts and redaction through both paths; compare saved config | PARTIAL: actual Chrome drafts/reload/restoration and direct API 428/412/400/save/readback PASS | Full production management-write path remains unexecuted; password redaction/preservation passed |
| C4 deferred network change | Approved idle Wi-Fi off/on | Observe complete response ACK before application, aborted response cancels, recheck idle; restore with Console | BLOCKED reachability: `http-wifi-capture-1` GET failed before any PUT | No deferred HTTP ACK or aborted-response cancellation claimed; authorization is granted |
| D1 real DHCP reassignment | Router/lease access and packet approval | Record old/new lease, same name/CA/server fingerprint/device; peer resolution and device advertisements use new IP, no duplicate submission | BLOCKED exact router administration/lease access; shared Wi-Fi/DHCP confirmed | No lease reassignment or LAN configuration change made |
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | PARTIAL/FAIL peer evidence: `console-wifi-capture-1` off/on and device recovery passed; no TTL0 observed | One goodbye attempt is not reception; NSS failed before/during/after, so cache expiry unproven |
| D3 actual link loss | Approved Pico-only link fault | No successful goodbye claim after loss; record stale-peer TTL, recovery/address/state, existing job/owner unchanged | BLOCKED exact Pico-only link-fault mechanism/router access | Orderly Console disable does not test actual unexpected link loss |
...
| E2 alias conflict during job | Approved second board or exact alternative responder | Certified alias conflict; record actual winner, loser conflict/empty advertised name/no suffix; finite inhibited job continues | BLOCKED baseline peer reachability for safe conflict admission; no responder started | Host alternative remains narrower than E1; broad test authorization recorded |
| E3 conflict recovery | E2 + approved idle retry | Remove conflicting claimant, WIFI OFF/ON, same certified name probes active and owner remains coherent | BLOCKED E2; ordinary Wi-Fi recovery alone does not close conflict recovery | No conflict was injected |
...
| F2 client replacement | Separate new controller/browser/import approval | Rotate after authoritative idle cleanup; replacement works, old valid identity still trusted (issuance is not revocation) | PARTIAL: replacement controller/browser leaf WTP/HTTPS reads passed; original-again passed | No new Chrome identity import or production principal-rotation run |
| F3 identity failures | Approved bounded negative TLS requests | Wrong hostname/CA, missing/invalid/expired client, IP without SAN fail at intended client/device layer; no WTP mutation | PARTIAL: wrong name/CA and absent IP SAN rejected; positive controls passed; negative clients closed without HTTP | Missing/untrusted/expired clients returned unexpected record-layer errors; intended alert oracle failed and remains open |
...
| F6 literal IP SAN success | Approved exact IP-SAN image/client URL | Exact current IP SAN authenticates browser URL and numeric HTTP authority | PARTIAL: exact IP-SAN TLS/WTP and numeric HTTP authority passed; `ip-san-clients-1` | Actual Chrome literal-IP URL unexecuted after Mac relocked; original no-IP-SAN image restored |
| G1 finite completion | Approved bounded host and direct-browser jobs | Complete CAPS-valid LOAD before ARM; matching job terminal STATUS and explicit inactive output | PARTIAL: six-second direct completion and actual finite production QRSS complete; independent terminal/cleanup proof | Actual Chrome submission unexecuted; host wrapper attempt 5 exit 1 retained alongside proven runtime result |
| G2 cancellation/foreign owner | Approved bounded jobs and owner mutations | Loaded/Armed/Running ABORT where supported; foreign ABORT/RELEASE rejected; explicit owner cleanup | PASS direct Loaded/Armed/Running cancellations and foreign HTTPS rejection; `jobs-attempt-1` | No implication that Chrome or production cancellation subcases passed |
...
| G4 lost LOAD/ARM/ABORT replies | Audited bounded response-loss mechanism | Prove intended request reached device and reply was withheld; original request ID/bytes replay or STATUS reconciliation, no duplicate execution | BLOCKED reachability: Mac and Linux prepared batches failed before WTP/CLAIM | No lost-reply case executed; proposed test application-boundary withholding is not wire-level loss |
| G5 failed DNS/TLS reconnect | Approved isolated controller fault | Retain output unknown and failed history; no additional LOAD/ARM; restore and reconcile using original identity | BLOCKED controller fault admission while baseline reachability fails | Natural connection failures retained; not the designed production reconciliation case |
| G6 process/boot/device changes | Approved test-process restart, board restart/identity mechanism | New process session cannot adopt old work; boot/device change blocks resubmission and retains failures | BLOCKED baseline network admission for prepared production restart packet; reflash boot changes separately observed | No new-process nonadoption or production boot-change latch claimed |
| G7 USB versus Console recovery | Explicit owned USB recovery and Console override | USB principal cannot steal TLS job; separately approved Console ABORT suspends scheduling and confirms disable/release | BLOCKED TLS-owner setup unavailable; prepared USB/Console packet not invoked | Read-only USB is not ownership recovery proof |
...
| H2 final cleanup | Approved cleanup set + read-only verification | Exact image/boot/device, inactive STATUS/unowned, scheduling disabled/history preserved, Wi-Fi/name and client/trust/service states recorded | PASS executed-scope USB/service cleanup in `final-state.*`; current peer reachability remains unresolved | Original image/config/trust retained; service active, hashes unchanged, provider output disabled; PID changed with approved pauses |
