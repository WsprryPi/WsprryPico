...
| D1 real DHCP reassignment | Router/lease access and packet approval | Record old/new lease, same name/CA/server fingerprint/device; peer resolution and device advertisements use new IP, no duplicate submission | OPEN: temporary Pico-only assignment saved, but LAN Apply returned HTTP 400 and no new address was obtained; reservation deleted | [D1 execution](phase11-4-d1-results.md); actual production baseline and ordinary-DHCP cleanup PASS, new-address acceptance unexecuted |
