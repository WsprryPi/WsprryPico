...
| D1 real DHCP reassignment | Controlled AP and independent peer; approved corrected image | Record old/new lease, same name/CA/server fingerprint/device; peer resolution and device advertisements use new IP, no duplicate submission | PASS bounded Linux reference and actual production client: same-boot .10 → .20, current native resolution and authenticated WTP/HTTPS within unchanged deadlines; full item OPEN for native Mac/Chrome | [Corrected-image execution](phase11-4-radio-results.md): approved A flash, verified Bohica-IoT/host restoration; no LOAD/ARM |
