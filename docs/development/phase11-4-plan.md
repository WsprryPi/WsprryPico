...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish a successful sample from reliability | Intermittent PASS/FAIL historically; final G4–G7 Mac and Linux resolution PASS | Reliability remains open; [latest bounded observations](phase11-4-g4-g7-results.md) do not erase earlier timeouts |
...
| E2 alias conflict during job | Approved second board or exact alternative responder | Certified alias conflict; actual winner/loser, no suffix, finite inhibited job continues | OPEN: conflict responder and job case not executed; latest bounded peer admission passes | Alternative responder remains narrower than E1; earlier reachability failures retained |
...
| H2 final cleanup | Approved cleanup set + read-only verification | Exact firmware/boot/device, inactive/unowned, disabled schedules and preserved saved state; clients/service recorded | PASS executed G4–G7 scope: fresh USB plus Mac/Linux HTTPS and resolution, installed service active/provider inactive | [Latest cleanup](phase11-4-g4-g7-results.md); broader reliability remains open |
