...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | PARTIAL: additional bounded recovery/HTTPS checks pass; historical intermittent TCP/NSS failure remains unexplained | [B2/D2 execution](phase11-4-b2-d2-results.md); normal Chrome navigation works with Anti-tracker enabled; automated navigation is separate |
...
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | PARTIAL overall: full bounded packet/peer cases pass; corrected repeat capture also missed one required probe despite peer recovery | [B2/D2 execution](phase11-4-b2-d2-results.md); one-second cache-grace test defect fixed; historical recovery failure retained |
