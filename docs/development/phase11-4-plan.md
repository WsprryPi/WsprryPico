...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | FAIL reproduced: two new bounded ARP/TCP/NSS recovery failures; later recovery retained | [Controlled same-SSID run](phase11-4-same-ssid-results.md): recovery succeeds once with service paused; earlier intermittent failures remain unexplained |
