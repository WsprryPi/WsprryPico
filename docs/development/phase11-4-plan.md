...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | OPEN: packet trace matches received ARP request and successful Pico reply submission, but no client receipt in the failed window; final peer cleanup passed | [Packet-boundary record](phase11-4-packet-trace-results.md): delivery loss after linkoutput remains unresolved; earlier prolonged failures retained |
...
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | OPEN: earlier missing goodbyes retained; new delivered goodbye followed by stage-16 station-disable watchdog | [Packet-boundary record](phase11-4-packet-trace-results.md): no complete new cycle or repair; [earlier repeated harness](phase11-4-loop-results.md) retained |
