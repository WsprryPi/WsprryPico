...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | OPEN: valid active and idle recovery failures reproduced; received requests and successful reply submissions absent at Linux; separate IoT path also failed | [Shutdown/path record](phase11-4-shutdown-results.md): mixed eight-attempt campaign, delivery loss unresolved; earlier failures retained |
...
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | OPEN: eight new captured goodbyes, but only three full passes; three recovery failures and two invalid attempts; historical shutdown watchdog not reproduced or explained | [Shutdown/path record](phase11-4-shutdown-results.md): exact internal markers deployed; historical missing-goodbye/watchdog failures retained |
...
| E1 distinct boards/trust | Two separately authorized boards/CAs | Independent default names/device IDs and per-board trust rejection | NOT RUN: user reports a second Pico connected to wspr5 for later E1; identity/provisioning uninspected | Keep it outside the original serial-bound B2/D2 campaign; two-board testing remains separate |
...
| H2 final cleanup | Approved cleanup set + read-only verification | Exact firmware/boot/device, inactive/unowned, disabled schedules and preserved saved state; clients/service recorded | PASS bounded final cleanup: native Mac/Linux DNS and HTTPS plus USB authority verified; original host settings/recovery and installed provider state intact | [Latest cleanup](phase11-4-shutdown-results.md): same diagnostic image/boot; stable quiet memory baseline, temporary profile removed and second Pico untouched |
