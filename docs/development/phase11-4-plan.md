...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | OPEN: repeated campaign reproduced nine failed peer recoveries while USB/NTP remained responsive; final peer cleanup passed | [Repeated harness](phase11-4-loop-results.md): fixed Bohica client path; both clients affected during the prolonged window; loss location unresolved |
...
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | OPEN: missing goodbye reproduced with stale lookups through 80 seconds; other full cycles passed; original watchdog unresolved | [Repeated harness](phase11-4-loop-results.md): five full cycles, two physical passes and three retained failures, including one explained clock-gated TLS reset |
...
| H2 final cleanup | Approved cleanup set + read-only verification | Exact firmware/boot/device, inactive/unowned, disabled schedules and preserved saved state; clients/service recorded | PASS bounded final cleanup: native Mac/Linux DNS and HTTPS plus USB authority verified; original host settings and recovery intact | [Latest cleanup](phase11-4-loop-results.md): same diagnostic image/boot, preserved settings, no test processes and only new failed-unit bookkeeping cleared |
