...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | FAIL reproduced: two new bounded ARP/TCP/NSS recovery failures; later recovery retained | [Repeat and diagnosis](phase11-4-b2-d2-repeat-results.md); same-SSID comparison invalidated by host recovery service; normal Chrome navigation remains separate |
...
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | FAIL reproduced: goodbye delivery/native withdrawal failed and recovery failures recurred | [Repeat and diagnosis](phase11-4-b2-d2-repeat-results.md); previous passing cases and missing-probe capture retained; no reliability closure |
