...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish a successful sample from reliability | Intermittent PASS/FAIL reproduced in D2; later Mac/Linux resolution and authenticated reads PASS, Chrome reload failed | [Latest observations](phase11-4-d1-d3-results.md); reliable recovery and Chrome ERR_ADDRESS_UNREACHABLE remain open |
...
| D1 real DHCP reassignment | Router/lease access and packet approval | Record old/new lease, same name/CA/server fingerprint/device; peer resolution and device advertisements use new IP, no duplicate submission | BLOCKED scoped operation: signed-in Orbi RBR850 inspected; only reservation control exposed, temporary Pico-only exception pending | [D1–D3 record](phase11-4-d1-d3-results.md); no router mutation or actual address change |
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | PARTIAL: captured Pico A/PTR goodbye, native Mac removal/re-add, Linux negative lookups and final reprobe/announcement PASS; repeated recovery failed once | [D1–D3 record](phase11-4-d1-d3-results.md); retain TCP/NSS failure and later Chrome address error; root cause unresolved |
| D3 actual link loss | Approved Pico-only link fault | No successful goodbye claim after loss; record stale-peer TTL, recovery/address/state, existing job/owner unchanged | BLOCKED selective radio fault: Orbi web UI delegates access control to phone app; user-assisted shielding arrangement pending | [D1–D3 record](phase11-4-d1-d3-results.md); no actual unexpected link-loss or natural stale-TTL expiry claimed |
...
| H2 final cleanup | Approved cleanup set + read-only verification | Exact firmware/boot/device, inactive/unowned, disabled schedules and preserved saved state; clients/service recorded | PASS device/service cleanup: same-boot USB inactive/unowned, saved state intact, Mac/Linux authenticated reads and resolution, installed service unchanged/provider inactive | [Latest cleanup](phase11-4-d1-d3-results.md); clock holdover recorded, Chrome address error and repeated-recovery reliability remain open |
