...
| B2 Linux NSS mDNS | Candidate + wspr5 NSS | Bound getent/getaddrinfo; retain failures and distinguish successful samples from reliability | OPEN: intermittent ARP/TCP/NSS failures retained; three controlled IoT baselines failed, including USB power saving OFF | [Withdrawal investigation](phase11-4-network-withdrawal-results.md): exact deployed candidate recorded; no reliability closure |
...
| D2 orderly disable | Approved idle Wi-Fi off/on, capture | Compare goodbye attempt counter with captured TTL0 and measured peer cache expiry; reprobe same name | OPEN: diagnostic image captured goodbye and negative-cache checks, but peer recovery failed; original watchdog unresolved | [Withdrawal investigation](phase11-4-network-withdrawal-results.md): captured TTL-zero A/PTR and six negative lookups; nine recovery failures retained; paired short recovery and 20 local lifecycle cycles passed |
...
| H2 final cleanup | Approved cleanup set + read-only verification | Exact firmware/boot/device, inactive/unowned, disabled schedules and preserved saved state; clients/service recorded | PASS bounded final cleanup: restored host; normal diagnostic boot; USB inactive/unowned; native Mac/Linux resolution and authenticated HTTPS passed | [Prior cleanup](phase11-4-b2-d2-results.md) passed; [current investigation](phase11-4-network-withdrawal-results.md) records restoration from watchdog recovery to normal diagnostic firmware, preserved settings and host restoration |
