...
| P12.6: production integration | **Partial.** BLE provisioning/local control and the SoftAP browser path are production-wired. The basic target SoftAP control path has bounded native-Pi evidence. The Field-GATT/1 source contract is frozen and vector-checked across firmware, Bluefy, the native-Pi client and documentation. Candidate A has bounded RF-inhibited native-Pi maximum-profile and repaired Bluefy prepared-file activation evidence through a normal generation-3 restart and BLE readback. Generation-3 positive mTLS, SoftAP credential provisioning, reset administration and broad target acceptance remain open. |
