"""Frozen failed B8 evidence: scope preservation and adversarial rejection."""
import json
import shutil
import sys
import tempfile
...
from audit_phase11_5_r3_v2_browser import audit
from audit_phase11_5_r3_v2_browser_observer import audit as strict_observer

SOURCE=Path(__file__).resolve().parents[1]/'build/phase11-5-r3-v2-browser-b8/evidence'

@unittest.skipUnless(SOURCE.exists(),'Private B8 evidence required')
class B8EvidenceTests(unittest.TestCase):
    def test_intact_original_failure_and_mutations(self):
        expected=audit(SOURCE)
        self.assertFalse(expected['cases'][0]['cancellation_verified'])
        self.assertFalse(expected['cases'][0]['timing_acceptance'])
        self.assertEqual(expected['cadence']['info']['samples'],900)
        ...
        changes=[
            lambda r:(r/'case-3-armed.png').write_bytes(b''),
